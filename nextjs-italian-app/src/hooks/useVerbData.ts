...
export const useVerbData = () => {
  ...
  const fetchRandomPronoun = useCallback(async (): Promise<string> => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/pronouns/random/subject`, {
      ...
      return fallbackPronouns[Math.floor(Math.random() * fallbackPronouns.length)];
    } finally {
      setIsLoading(false);
    }
  }, [API_BASE_URL]);
  ...
  const fetchRandomVerb = useCallback(async (filterOptions: FilterOptions = {}): Promise<VerbData> => {
    ...
    try {
      ...
      if (regularOnly === true) {
        const allRegularVerbs = [...fallbackVerbs.are, ...fallbackVerbs.ere, ...fallbackVerbs.ire];
        return allRegularVerbs[Math.floor(Math.random() * allRegularVerbs.length)];
      }

      // Otherwise return any verb
      const allVerbs = [...fallbackVerbs.are, ...fallbackVerbs.ere, ...fallbackVerbs.ire];
